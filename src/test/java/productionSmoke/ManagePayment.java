...
public class ManagePayment extends BaseTestUser {
	...
	public void ManagePayment() throws InterruptedException {
		test = rep.createTest("ManagePayment");
		test.log(Status.INFO, "Add Payment Card to User ");
		Verifyloginregister();
		homepage.ClickonMainLogo();
		menupage.ClickonUserName();
		menupage.ClickonManagePayment();
		menupage.VerifyPaymentPagetitle();
		menupage.ClickonPaymentAddnewCard();
		menupage.PaymentPage(CardNumber, ExpiryMonth, ExpiryYear,CVV, NameonThecard);
		if(menupage.VerifyCardAdded())
		{
			test.log(Status.PASS, "Card added Successfully ,Card details are: Card Number :" +"<b>" +CardNumber );
		}
		else
		{
			test.log(Status.FAIL, "Due to some technical issues,unable to add card" );
		}
		
		if(menupage.removeCard())
		{
			test.log(Status.PASS, "Added Card Removed Successfully");
		}
		else
		{
			test.log(Status.FAIL, "Due to some technical issues,unable to remove card" );
		}
		driver.close();
	}
