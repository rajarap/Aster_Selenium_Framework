...
public class AddInsurance extends BaseTestUser {
	...
	@Test(priority = 1)
	public void VerifyAddInsurance() throws InterruptedException {
		test = rep.createTest("VerifyAddInsurance");
		test.log(Status.INFO, "Add Insurance to User ");
		Verifyloginregister();
		homepage.ClickonMainLogo();
		menupage.ClickonMenu();
		menupage.ClickonInsurance();
		menupage.ClickonInsuranceAddnew();
		menupage.VerifyInsurancePagetitle();
		menupage.SelectInsuranceProvider(Insuranceprovider);
		Random rand = new Random();

		 // nextInt as provided by Random is exclusive of the top value so you need to add 1 

		int num = rand. nextInt(9000000) + 1000000;
		  String InsuraceID=Integer.toString(num);
		String ins=InsuraceID;
		menupage.EnterInsuranceID(ins);
		menupage.EnterexpiryDate();
		menupage.ConsentCheck();
		menupage.clickonInsuranceAdd();
		menupage.verifyInsuranceSuccessMessage();
		menupage.GoToInsurance();
		String Insuranceinfo=menupage.ValidateInsuranceDetails(ins);
		if(Insuranceinfo.equals(""))
		{
			test.log(Status.FAIL, "Due to some technical issues Insurance not added");
		}
		else
		{
			test.log(Status.PASS, "Insurance added Successfully ,Insurance details are:"+"<b>" +Insuranceinfo );
		}
		driver.close();
